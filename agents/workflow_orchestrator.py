...
@dataclass
class Document:
    """Document representation throughout pipeline"""
    id: str
    path: Path
    type: DocumentType
...
class WorkflowOrchestrator:
    ...
    Coordinates all agents and manages document flow
    """
    ...
    def __init__(self, config_path: str):
        ...
        self.primary_model = self.config.get('primary_embedding_model', 
                                            'sentence-transformers/all-MiniLM-L6-v2')
        self.fallback_model = self.config.get('fallback_embedding_model',
                                             'jhgan/ko-sbert-nli')
    ...
    async def process_document(self, document: Document) -> Document:
        ...
        Returns:
            Processed document with all stages completed
        """
        start_time = datetime.now()
        
        try:
            # Stage 1: Parsing (if needed)
    ...
    async def _embed_chunks(self, document: Document) -> Document:
        ...
        for chunk in document.chunks:
            task = asyncio.create_task(
                self._generate_embedding(chunk['text'], chunk['id'])
            )
            tasks.append(task)
        ...
        for i, emb in enumerate(embeddings):
            if isinstance(emb, Exception):
                ...
                document.metadata.error_messages.append(
                    f"Embedding failed for chunk {i}"
                )
            else:
    ...
    async def batch_process(self, documents: List[Document]) -> List[Document]:
        ...
        # Process error queue with retries
    ...
    async def _process_error_queue(self):
        """Process documents in error queue with retries"""
        retry_count = {}
    ...
    async def _check_for_new_documents(self) -> List[Document]:
        ...
        # This would typically check a directory or database
