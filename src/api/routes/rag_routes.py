...
class RAGRouter:
    ...
    def __init__(self, rag_pipeline: RAGPipeline, error_handler: ErrorHandler):
        """
    ...
    def _setup_routes(self):
        ...
        async def upload_file(file: UploadFile = File(...)):
            ...
            try:
                ...
                error_info = self.error_handler.log_error(e, {"filename": file.filename})
                raise HTTPException(
                    status_code=500, detail=f"File upload failed: {error_info['error_message']}"
                )
        ...
        async def upload_documents(request: DocumentUploadRequest):
            ...
            try:
                ...
                error_info = self.error_handler.log_error(
                    e, {"file_paths": request.file_paths, "metadata": request.metadata}
                )
                raise HTTPException(
                    status_code=500, detail=f"Document upload failed: {error_info['error_message']}"
                )
        ...
        async def process_query(request: QueryRequest):
            ...
            try:
                ...
                context_chunks = self.rag_pipeline.retrieve(request.query, top_k=request.top_k)
                ...
                # Generate response
                response = self.rag_pipeline.generate_response(context_chunks, request.query)

                # Extract citations
                citations_result = self.rag_pipeline.extract_citations(context_chunks, response)
                ...
                error_info = self.error_handler.log_error(
                    e, {"query": request.query, "filters": request.filters}
                )
                raise HTTPException(
                    status_code=500,
