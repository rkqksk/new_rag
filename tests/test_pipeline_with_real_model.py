    ...
    async def test_multiple_consultations(self):
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                # 데이터 생성
                response, is_high_quality = await self.teacher_service.generate_training_data(
                    request
                )

                logger.info(
                    f"  신뢰도: {response.confidence:.2f} | 품질: {response.quality_score:.2f}"
                )
                ...
                if eval_result.is_high_quality:
                    ...
                    logger.info(
                        f"  → 훈련 데이터로 추가됨 (누적: {len(self.high_quality_samples)})"
                    )
                else:
    ...
    def _export_training_data(self):
        ...
        try:
            ...
            output_file = exporter.export_to_json(
                self.high_quality_samples, "./exports/validated_training_data.json"
            )
