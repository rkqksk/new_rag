...
class PerformanceBenchmarks:
    ...
    def sample_documents(self, tmp_path):
        ...
        documents = [tmp_path / f"doc_{i}.txt" for i in range(100)]
    ...
    def test_document_ingestion_performance(self, benchmark, sample_documents):
        """
    ...
    def test_query_retrieval_performance(self, benchmark, sample_documents):
        """
        ...
        def query_func():
            return pipeline.retrieve("performance benchmark query", top_k=10)

        results = benchmark(query_func)
        assert len(results) > 0

    @pytest.mark.parametrize("document_count", [10, 100, 1000])
    def test_scalability(self, benchmark, tmp_path, document_count):
        """
        Test RAG pipeline scalability with increasing document volumes
        """
        documents = [tmp_path / f"scalability_doc_{i}.txt" for i in range(document_count)]
