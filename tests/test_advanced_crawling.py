...
async def test_static_crawler_retry():
    """Test retry logic"""
    config = StaticCrawlerConfig(timeout=5, max_retries=2, retry_delay=0.5)
...
async def test_dynamic_crawler_javascript():
    ...
    async with DynamicCrawler(config) as crawler:
        ...
        result = await crawler.crawl("http://httpbin.org/html", wait_for_timeout=1000)
...
async def test_multi_strategy_crawl_static():
    ...
    async with MultiStrategyCrawler() as crawler:
        result = await crawler.crawl("http://httpbin.org/html", method=CrawlMethod.STATIC)
...
async def test_auth_manager_basic():
    ...
    credentials = AuthCredentials(username="test_user", password="test_pass")

    client = await auth_manager.authenticate(AuthType.BASIC, credentials)
...
async def test_auth_manager_api_key():
    ...
    credentials = AuthCredentials(api_key="test-api-key-12345", api_key_header="X-API-Key")

    client = await auth_manager.authenticate(AuthType.API_KEY, credentials)
...
def test_auth_manager_totp_uri():
    ...
    uri = AuthenticationManager.get_totp_provisioning_uri(
        secret=secret, name="test@example.com", issuer="TestApp"
    )
...
def test_evasion_manager_headers():
    ...
    config = EvasionConfig(rotate_user_agent=True, randomize_headers=True, spoof_referrer=True)
    manager = AntiDetectionManager(config)
...
async def test_evasion_manager_delay():
    ...
    config = EvasionConfig(min_delay=0.1, max_delay=0.2, randomize_delay=True)
    manager = AntiDetectionManager(config)
...
def test_excel_processor_merged_cells():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        processor = AdvancedExcelProcessor(
            str(test_file), ExcelProcessorConfig(extract_merged_cells=True)
        )
        data = processor.extract_all()
...
def test_excel_processor_formulas():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        processor = AdvancedExcelProcessor(
            str(test_file), ExcelProcessorConfig(extract_formulas=True, data_only=False)
        )
        data = processor.extract_all()
...
async def test_full_crawl_pipeline():
    ...
    config = CrawlConfig(
        method=CrawlMethod.STATIC, use_evasion=True, rate_limit_requests=5, rate_limit_window=10.0
    )
