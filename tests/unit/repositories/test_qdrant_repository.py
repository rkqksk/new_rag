...
class TestQdrantRepository:
    ...
    async def test_search_basic(self, repository, mock_qdrant_client, sample_search_results):
        ...
        mock_qdrant_client.search.return_value = [
            MagicMock(id=result["id"], score=result["score"], payload=result["payload"])
        ...
        results = await repository.search(
            collection_name="products", query_vector=query_vector, limit=10
        )
    ...
    async def test_search_with_score_threshold(self, repository, mock_qdrant_client):
        ...
        # Act
        results = await repository.search(
            collection_name="products", query_vector=query_vector, limit=10, score_threshold=0.8
        )
    ...
    async def test_search_multi_vector(self, repository, mock_qdrant_client):
        ...
        query_vectors = {"text": [0.1] * 384, "image": [0.2] * 1024, "shape": [0.3] * 128}
    ...
    async def test_get_point(self, repository, mock_qdrant_client):
        ...
        mock_qdrant_client.retrieve.return_value = [
            MagicMock(id="PROD-001", payload={"text": "50ml PET 용기"})
        ]

        # Act
        result = await repository.get_point(collection_name="products", point_id="PROD-001")
    ...
    async def test_upsert_points(self, repository, mock_qdrant_client):
        ...
        # Act
        success = await repository.upsert_points(collection_name="products", points=points)
    ...
    async def test_search_empty_results(self, repository, mock_qdrant_client):
        ...
        # Act
        results = await repository.search(
            collection_name="products", query_vector=[0.1] * 384, limit=10
        )
