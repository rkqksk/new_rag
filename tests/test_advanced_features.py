...
class TestAdvancedFeatures:
    def test_metadata_date_filtering(self):
        ...
        filtered_docs = MetadataFilter.filter_by_date(
    ...
    def test_metadata_tag_filtering(self):
        ...
        # Include tech tag
        include_filtered = MetadataFilter.filter_by_tags(
            documents, include_tags=['tech']
        )
        assert len(include_filtered) == 1
        assert include_filtered[0]['text'] == 'Technical Document'

        # Exclude business tag
        exclude_filtered = MetadataFilter.filter_by_tags(
            documents, exclude_tags=['business']
        )
    ...
    def test_metadata_transformation(self):
        ...
        documents = [
            {
                ...
                'metadata': {
                    'old_field': 'value',
                    'another_old_field': 'another_value'
                }
            }
        ]
        ...
        transformed_docs = MetadataFilter.transform_metadata(
