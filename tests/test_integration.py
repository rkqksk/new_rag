...
class TestRAGIntegration:
    def test_document_upload(self, test_client, tmp_path):
        ...
        response = test_client.post(
            "/rag/upload", json={"file_paths": [str(test_doc)], "metadata": {"source": "test"}}
        )
    ...
    def test_query_processing(self, test_client, tmp_path):
        ...
        test_docs = [tmp_path / "doc1.txt", tmp_path / "doc2.txt"]
        for doc in test_docs:
            doc.write_text("This is a test document about machine learning and AI.")

        test_client.post("/rag/upload", json={"file_paths": [str(doc) for doc in test_docs]})
    ...
    def test_error_handling(self, test_client):
        ...
        # Invalid query
        response = test_client.post("/rag/query", json={"query": ""})
        assert response.status_code == 422  # Validation error

        # Non-existent document upload
        response = test_client.post(
            "/rag/upload", json={"file_paths": ["/path/to/nonexistent/file.txt"]}
        )
