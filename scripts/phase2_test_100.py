#!/usr/bin/env python3
"""
PHASE 2 TEST: Extract Details for 100 Products
Uses the proven extraction method from onehago_product_crawler.py
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'/Users/oypnus/Project/rag-enterprise/data/onehago/crawled/logs/phase2_test_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
INPUT_FILE = BASE_PATH / 'crawled' / 'test_phase1_urls.jsonl'
OUTPUT_DIR = BASE_PATH / 'crawled' / 'test_phase2_results'
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                details['full_images'] = [
                    img for img in executor.map(
                        lambda url: self.download_image(url, product_id),
                        image_urls
                    ) if img is not None
                ]
...
def main():
    logger.info("="*70)
    logger.info("🧪 PHASE 2 TEST: Extract Details for 100 Products")
    logger.info("="*70)
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"✅ PHASE 2 TEST COMPLETE")
    logger.info(f"{'='*70}")
