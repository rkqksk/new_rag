#!/usr/bin/env python3
"""
🚀 FREEMOLD.NET PHASE 2 - AUTHENTICATED TEXT EXTRACTION (Using Existing Session)
================================================================================

Extracts product text data using the authenticated browser session that user already logged into.
- Uses cookies from the authenticated session (preserved from manual login)
...
Key Insight: User has already logged in via the Chrome browser opened by freemold_manual_browser_login.py
We must use those authenticated cookies, not try to create a new session.
"""
...
# ============================================
# LOAD COOKIES FROM AUTHENTICATED SESSION
# ============================================

def load_cookies():
    """Load cookies from the authenticated browser session"""
    if COOKIES_FILE.exists():
        with open(COOKIES_FILE, 'r') as f:
            ...
            return cookies
    logger.warning("⚠️  cookies.json not found - will attempt extraction anyway")
    return {}
...
# ============================================
# EXTRACT PRODUCT TEXT DATA (AUTHENTICATED)
# ============================================

def extract_product_text(product_url, product_id, category, category_name, session):
    """
    ...
    try:
        # Use authenticated request with session
        ...
        if '비회원은' in response.text or 'alert' in response.text.lower():
            logger.warning(f"⚠️  Product {product_id}: Got login error or non-member alert")
            return product_data
