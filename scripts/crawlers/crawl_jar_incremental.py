#!/usr/bin/env python3
"""
Jar 카테고리 증분 업데이트 크롤러
기존 제품 SKIP, 새 제품만 크롤링

페이지네이션: 단순 (1-4 페이지, 단일 그룹)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('crawl_jar_incremental.log'),
...
async def main():
    ...
    logger.info(f"\n{'='*80}")
    logger.info("Jar 카테고리 증분 크롤링 시작")
    logger.info(f"{'='*80}")
    ...
    summary_path = crawler.output_dir / f"category_Jar_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(summary_path, 'w', encoding='utf-8') as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    logger.info(f"\n{'='*80}")
    logger.info("✅ Jar 크롤링 완료!")
    logger.info(f"{'='*80}")
