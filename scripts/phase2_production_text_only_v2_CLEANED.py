#!/usr/bin/env python3
"""
PHASE 2 PRODUCTION V2: Extract ALL Product Text Data (NO IMAGES) - OPTIMIZED
Uses CLEANED and DEDUPLICATED data from Phase 1 cleanup
Input: cleaned/all_products_clean.jsonl (22,901 unique products - 87x efficiency improvement)
Storage: ~23MB for 22,901 products (text only)
...
# Setup logging
log_file = Path('/Users/oypnus/Project/rag-enterprise/data/onehago/crawled/logs') / f'phase2_production_text_v2_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
# Paths - FIXED TO USE CLEANED DATA
BASE_PATH = Path('/Users/oypnus/Project/rag-enterprise/data/onehago')
INPUT_FILE = BASE_PATH / 'crawled' / 'production' / 'cleaned' / 'all_products_clean.jsonl'  # ⭐ CORRECTED
OUTPUT_DIR = BASE_PATH / 'crawled' / 'production' / 'products_text_only_v2_cleaned'
PROGRESS_FILE = OUTPUT_DIR / 'phase2_v2_progress.json'
...
CHECKPOINT_INTERVAL = 1000  # Save progress every 1000 products (smaller for cleaner data)

class Phase2TextExtractor:
    def __init__(self):
        # Configure session with larger connection pool for 8 workers
...
def main():
    logger.info("="*80)
    logger.info("🚀 PHASE 2 PRODUCTION V2: Extract Product Text (CLEANED DATA - 87x Efficiency)")
    logger.info("="*80)
    ...
    logger.info("  Using CLEANED/DEDUPLICATED data from Phase 1")
    logger.info("  Input: 22,901 unique products (vs 2,011,553 duplicated)")
    logger.info("  Efficiency gain: 87x faster, no redundant crawling")
    logger.info("="*80)
    ...
    logger.info(f"✅ Loaded {total_products:,} unique product URLs")
    ...
    elapsed = datetime.now() - extractor.start_time
    logger.info(f"\n{'='*80}")
    logger.info(f"✅ PHASE 2 PRODUCTION V2 COMPLETE")
    logger.info(f"{'='*80}")
    ...
    logger.info(f"")
    logger.info(f"✨ EFFICIENCY ACHIEVEMENT:")
    logger.info(f"  Processed: 22,901 unique products (not 2,011,553 duplicates)")
    logger.info(f"  Time saved: ~87x faster than old approach")
    logger.info(f"  Storage saved: ~87x less redundant data")
    logger.info(f"{'='*80}")
