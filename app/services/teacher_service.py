...
다음 정보를 참고하여 답변하세요."""
        ...
        system_prompt = system_prompts.get(
            consultation_type,
            "당신은 고객 상담 전문가입니다. 친절하고 정확한 답변을 제공해주세요."
        )
...
"""
    ...
    def _parse_teacher_response(self, full_text: str) -> Tuple[str, str]:
        ...
        for line in parts:
            ...
            if reasoning_started and any(keyword in line for keyword in ["추론", "reasoning", "왜", "이유", "because"]):
    ...
    async def _calculate_confidence(
        ...
        if rag_context:
            used_context_count = sum(
                1 for context in rag_context[:3]
    ...
    def _create_error_response(self, request: TeacherGenerationRequest) -> TeacherGenerationResponse:
        """오류 발생 시 기본 응답 생성"""
...
class TrainingDataExporter:
    ...
    def format_training_sample(
        teacher_response: TeacherGenerationResponse
    ) -> Dict[str, Any]:
        """
...
{rag_context_text}"""
    ...
    @staticmethod
    def export_to_json(
        samples: List[TeacherGenerationResponse],
        output_path: str
    ) -> bool:
        """
        ...
        try:
            training_data = [
                TrainingDataExporter.format_training_sample(sample)
