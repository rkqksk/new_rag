...
class SearchService:
    ...
    def __init__(self, qdrant_repo, redis_repo, postgres_repo=None):
        ...
        try:
            self.embedder = MultiModalEmbedder(
                enable_text=True, enable_image=True, enable_shape=False  # Enable if needed
            )
    ...
    async def search(
        self, query: str, session_id: Optional[str] = None, top_k: int = 20, use_cache: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.reranker and search_results:
                ...
                reranked = self.reranker.rerank(
                    query=query, results=results_for_rerank, top_k=top_k, text_field="text"
                )
            ...
            if self.personalization and session_id:
                ...
                self.personalization.track_search(
                    session_id=session_id, query=query, results_count=len(search_results)
                )

                # Apply personalization
                search_results = self.personalization.personalize_search_results(
                    session_id=session_id, results=search_results, query=query, top_k=top_k
                )
    ...
    async def image_search(
        self, image_path: str, session_id: Optional[str] = None, top_k: int = 20
    ) -> Dict[str, Any]:
        """
        Image-based search
