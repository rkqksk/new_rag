  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="전체 Webhook" value={stats.total}  />
        <StatCard title="활성 Webhook" value={stats.active}  changeType="neutral" />
        <StatCard title="성공률" value={`${stats.successRate}%`}  changeType="neutral" />
        <StatCard title="전체 호출" value={stats.totalCalls}  changeType="neutral" />
      </div>
              ...
              {logs.slice(0, 10).map((log) => {
                ...
                return (
                  <TableRow key={log.id} className="border-stone-900 hover:bg-stone-950">
                    ...
                    <TableCell className="text-stone-400 max-w-xs truncate">
                      {log.error || "-"}
                    </TableCell>
                  </TableRow>
                )
