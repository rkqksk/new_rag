...
**direnv 기반 자동 환경 활성화**
- `.direnv/python-3.11/`: 자동 생성 가상환경
- `.envrc`: 환경변수 및 Python 환경 설정
- ⚠️ `venv/`, `.venv/` 사용하지 않음 (삭제 가능)

```bash
# 프로젝트 진입 시 자동 활성화
cd /Users/oypnus/Project/rag-enterprise
# direnv가 자동으로 환경변수 로드 및 Python 3.11 활성화
...
**환경변수 구조 (.envrc)**:
- API Keys: CLAUDE_API_KEY, ANTHROPIC_API_KEY, GROQ_API_KEY, OPENAI_API_KEY, OLLAMA_API_KEY
- Database: POSTGRES_USER, POSTGRES_PASSWORD, MONGO_USER, MONGO_PASSWORD
- N8N & Monitoring: N8N_USER, N8N_PASSWORD, GRAFANA_PASSWORD
- Claude SDK: ANTHROPIC_MODEL, CLAUDE_SDK_TIMEOUT

### Docker 인프라 스택 (Colima 환경)

**네트워크**: `rag_network` (172.28.0.0/16)

| 서비스 | IP | 포트 | 리소스 (CPU/RAM) | 용도 |
|--------|-----|------|------------------|------|
| Qdrant | 172.28.0.2 | 6333, 6334 | 2/4GB | 벡터 검색 |
| Redis | 172.28.0.3 | 6379 | 1/2GB | 캐싱 |
| PostgreSQL | 172.28.0.4 | 5432 | 2/4GB | 메타데이터 |
| N8N | 172.28.0.5 | 5678 | 2/3GB | 워크플로우 |
| Ollama | 172.28.0.6 | 11434 | 4/8GB | 로컬 LLM |
| FastAPI | 172.28.0.7 | 8000 | 2/4GB | API 서버 |

**볼륨**: `qdrant_data`, `redis_data`, `postgres_data`, `n8n_data`, `ollama_models`

**자세한 개발 환경 가이드**: `DEV_ENVIRONMENT.md` 참조
