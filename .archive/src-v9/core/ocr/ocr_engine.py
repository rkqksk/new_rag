...
class MultiEngineOCR:
    ...
    def __init__(
        ...
        logger.info(
            f"MultiEngineOCR initialized: PaddleOCR={use_paddle}, EasyOCR={use_easy}, Tesseract={use_tesseract}"
        )
    ...
    def _init_easy(self):
        ...
        try:
            ...
            return easyocr.Reader(["ko", "en"], gpu=self.use_gpu, verbose=False)
        except ImportError:
    ...
    def _check_tesseract(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Tesseract not available. Install with: apt-get install tesseract-ocr tesseract-ocr-kor"
            )
            return False

    def extract_text(self, image: Image.Image, enable_fallback: bool = True) -> OCRResult:
        """
        ...
        if self.paddle_ocr:
            ...
            logger.warning(
                f"PaddleOCR low confidence ({result.avg_confidence:.2f}), trying fallback..."
            )
        ...
        if self.easy_ocr and enable_fallback:
            ...
            logger.warning(
                f"EasyOCR low confidence ({result.avg_confidence:.2f}), trying Tesseract..."
            )
        ...
        return OCRResult(
            text_boxes=[], full_text="", avg_confidence=0.0, engine="none", processing_time_ms=0.0
        )
    ...
    def batch_extract(
        self, images: List[Image.Image], enable_fallback: bool = True
    ) -> List[OCRResult]:
        """
