...
class PostgresRepository:
    ...
    def __init__(
        self, host: str, port: int, database: str, username: str, password: str, pool_size: int = 20
    ):
    ...
    async def insert_search_event(
        self, session_id: str, query: str, keywords: List[str], results_count: int
    ) -> bool:
        """Insert search event"""
        try:
            await self.execute(
    ...
    async def insert_product_event(
        self, session_id: str, product_id: str, event_type: str, product_data: Dict[str, Any]
    ) -> bool:
