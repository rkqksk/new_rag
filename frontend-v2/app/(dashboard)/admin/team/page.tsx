...
export default function TeamManagementPage() {
  ...
  const getRoleColor = (role: string) => {
    const roleColors: Record<string, string> = {
      "super-user": "bg-purple-900 text-purple-100",
      "admin": "bg-blue-900 text-blue-100",
      "manager": "bg-cyan-900 text-cyan-100",
      "staff": "bg-green-900 text-green-100",
      "operator": "bg-emerald-900 text-emerald-100",
      "customer-vip": "bg-amber-900 text-amber-100",
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          icon="📧"
          changeType={stats.pendingInvites > 0 ? "neutral" : "positive"}
        />
        <StatCard
          title="역할 종류"
          value={stats.roles}
                    ...
                    <TableRow key={member.id} className="border-stone-900 hover:bg-stone-950">
                      ...
                      <TableCell>
                        {member.status === "active" ? (
                          <Badge className="bg-green-900 text-green-100">활성</Badge>
                        ) : member.status === "pending" ? (
                          <Badge className="bg-yellow-900 text-yellow-100">대기 중</Badge>
                        ) : (
